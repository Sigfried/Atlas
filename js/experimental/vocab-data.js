...
define(function (require, exports) {
	...
	var host = 'http://127.0.0.1:3031'
	var apiPath = '/api/chcos'
	var cdmSchema = 'cdm_v501'
	var resultsSchema = 'results'

	var loadedPromise = $.Deferred();
	loadedPromise.resolve();

  function catchApiErrs(err) {
    console.error(err)
    debugger
    throw new Error('stop here')
  }

	function loadStaticVocabData() {
    let url = `${host}${apiPath}/vocabularies?cdmSchema=${cdmSchema}&resultsSchema=${resultsSchema}`
    return fetch( url, {}).catch(catchApiErrs)
	}
	function loadStaticRelationshipsData() {
    let url = `${host}${apiPath}/vocabularies?cdmSchema=${cdmSchema}&resultsSchema=${resultsSchema}`
    return fetch( url, {}).catch(catchApiErrs)
	}
	function loadStaticConceptGroupsData() {
    let url = `${host}${apiPath}/vocabularies?cdmSchema=${cdmSchema}&resultsSchema=${resultsSchema}`
    return fetch( url, {}).catch(catchApiErrs)
	}
	function loadConceptInfo(concept_id) {
    let url = `${host}${apiPath}/conceptInfo?cdmSchema=${cdmSchema}&resultsSchema=${resultsSchema}&concept_ids=[${concept_id}]`
    return fetch( url, {}).catch(catchApiErrs)
	}
  ...
  return {
    loadStaticVocabData,
    loadStaticRelationshipsData,
    loadStaticConceptGroupsData,
    loadConceptInfo,
  }
